...
<div class="container p-4">
    ...
    <div class="p-2 mt-10 rounded-md">
        @if (count($productImages) < 5)
            <form method="post" action="{{ route('seller.productImage', $product->id) }}" class="dropzone" id="myDropzone">
                @csrf
            </form>
        @else
            <div class="alert alert-warning">You have reached the maximum number of images (5). Please delete some images to upload new ones.</div>
        @endif
    </div>
    <br>
    ...
    <div class="flex flex-wrap gap-3 p-4" id="image-gallery">
        ...
        <blockquote class="w-32 h-auto flex-wrap">
            <img alt="productimage" src="{{ baseUrl() . 'uploads/' . $img->images }}" class="aspect-square border w-full hover:text-black rounded-t-xl object-contain" />
            <form action="{{ route('seller.deleteImage', $img->id) }}" method="POST" id="delete-form-{{ $img->id }}">
                @csrf
                @method('delete')
                <button type="button" onclick="deleteSingleImage({{ $img->id }})" class="border bg-red-600 cursor-pointer openModal hover:bg-red-500 text-white text-center w-full">
                    Delete
                </button>
            </form>
        </blockquote>
