        ...
        @foreach (getCategories(0) as $category)
            <div class="mt-2 ml-2 ">
                <input type="checkbox" name="category" onchange="checkOnlyOne(this)"
                    ...
                    {{ $category->id == $product->category_id ? 'checked' : '' }} id="{{ $category->id }}"
                    value="{{ $category->id }}" {{-- {{ getCategories($category->id)->count() == 0 ? '' : 'disabled' }} --}}>
                <label for="{{ $category->id }}" class="ml-1">{{ $category->categoryname }}</label>


                <div class="ml-4 subcategory-container">
                    @foreach (getCategories($category->id) as $subcategory)
                        <div class="ml-7 mt-1">
                            <input type="checkbox" {{ $subcategory->id == $product->category_id ? 'checked' : '' }}
                                name="category" onchange="checkOnlyOne(this)"
                                class="w-[13px] h-[13px] text-blue-600 bg-gray-100 border-gray-300 rounded"
                                id="{{ $subcategory->id }}" value="{{ $subcategory->id }}" {{-- {{ getCategories($subcategory->id)->count() == 0 ? '' : 'disabled' }} --}}>
                            <label for="{{ $subcategory->id }}" class="ml-1">{{ $subcategory->categoryname }}</label>
                        </div>
                        @foreach (getCategories($subcategory->id) as $twosubcategory)
                            <div class="ml-20 mt-1">
                                <input {{ $twosubcategory->id == $product->category_id ? 'checked' : '' }}
                                    type="checkbox" name="category" onchange="checkOnlyOne(this)"
                                    class="w-[13px] h-[13px] text-blue-600 bg-gray-100 border-gray-300 rounded"
