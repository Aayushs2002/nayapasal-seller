...
<div class="max-w-md mx-auto text-center bg-white px-4 sm:px-8 py-10 rounded-xl shadow my-20">
    <header class="mb-8">
        ...
        <p class="text-[15px] text-slate-500">Enter the 5-digit verification code that was sent to your email address.</p>
    </header>
    <form action="{{ route('seller.checkresetotp',$token) }}" method="post" id="otp-form"
        class="items-center ">
        @csrf
        <div>
            <label class="block text-gray-700 font-bold my-2" for="enter otp">
                Enter OTP
            </label>

            <input name="otp" value="{{ old('otp') }}"
                class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                id="otp" type="number" placeholder="Enter OTP">
            @error('otp')
